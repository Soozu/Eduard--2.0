...
# Function to extract city, category, and budget information from user query
def extract_query_info(query_text, available_cities, available_categories, available_budgets=None):
    ...
    Args:
        ...
        available_budgets (list, optional): List of available budget categories

    Returns:
        tuple: (extracted_city, extracted_category, extracted_budget, cleaned_query, sentiment_info, budget_amount)
    """
    ...
    budget_amount = None
    sentiment_info = None  # We'll add a placeholder for this
    ...
    category_mapping = {
        "cafe": ["cafe", "coffee", "restaurant", "dining", "breakfast", "lunch", "dinner"],
        ...
        "natural attraction": ["nature", "natural", "outdoors", "mountain", "lake", "volcano", "falls", 
                               "waterfall", "things to do", "beach", "ocean", "sea", "river", "hiking", "trek", "forest"],
        "theme park": ["park", "amusement", "rides", "attraction", "entertainment", "fun", "thrill"],
        "museum": ["museum", "collection", "exhibit", "puzzle", "art", "gallery", "cultural", "artifacts"],
        "resort": ["resort", "retreat", "relax", "spa", "wellness", "stay", "accommodation", "beach resort"]
    }
    ...
    budget_terms = {
        ...
        "medium": ["moderate", "mid-range", "standard", "average"],
        "high": ["luxury", "expensive", "high-end", "premium", "deluxe"]
    }
    
    for budget_category, terms in budget_terms.items():
        if any(term in query_lower for term in terms):
            extracted_budget = budget_category
            break
    
    # Check for numeric budget amounts
    amount_match = re.search(r'(\d+)\s*pesos|\$\s*(\d+)|(\d+)\s*\$', query_lower)
    if amount_match:
        # Get first non-None group
        budget_groups = [g for g in amount_match.groups() if g is not None]
        if budget_groups:
            budget_amount = int(budget_groups[0])
...
def get_recommendations(query_text, tokenizer, model, embeddings, df, city=None, category=None, budget=None, budget_amount=None, top_n=3):
    """
    ...
    Args:
        ...
        budget (str, optional): Budget level to filter results by (low, medium, high)
        budget_amount (int, optional): Numeric budget amount to filter by
    ...
    if budget_amount is not None:
        ...
        try:
            ...
            logger.warning(f"Error filtering by budget amount: {e}")
        ...
        try:
            ...
            logger.warning(f"Error filtering by budget category: {e}")
